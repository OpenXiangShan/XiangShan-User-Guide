...
此外，目前 {{processor_name}} 将处理器核挂死自定义为严重错误状态，作为硅后判断挂死的重要手段。其中 ROB commit stuck 超时检查由自定义 CSR mcorepwr[1] 控制：mcorepwr 的地址为 0xBC0，bit 1 为 COMMIT_STUCK_CHECK_ENABLE，复位值为 0。软件将该位写 1 时开启 ROB commit stuck 超时检查，超时后上报 critical error；写 0 时关闭该检查，处理器不会因该类 commit stuck 超时上报 critical error。
